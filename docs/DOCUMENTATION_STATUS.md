...
**`R2-01` až `R2-06` jsou implementovány; kontrakt C15 existuje → `R2-07` je `READY` (neimplementováno); `R2-08` zůstává `NOT_READY` (čeká na dokončení R2-07). Kontraktní mapa R2 (C1–C15) je kompletní.**
...
**Přesný další kanonický krok:** `R2-01` až `R2-06` jsou implementovány a C15 existuje → **`R2-07` je `READY`**. Další krok je **implementace `R2-07 – Local-to-Account Data Migration`** (lokální idempotentní attach dle C15 §5–§6, seed exclusion, second-account testy, e2e anonymní-trénink → registrace → attach → push). Po R2-07 následuje `R2-08` (kritická E2E evidence + R2 Exit Review, vč. dlužné emulátorové runtime evidence). Implementace `R2-07` smí začít až po Ready kontrole a samostatném pokynu.
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. **Kontraktní mapa R2 (C1–C15) je kompletní** (C15 v `docs/12-data/r2-local-to-account-migration-contract.md`, `LAM-001..015`). **`R2-01` až `R2-06` jsou implementovány**; **`R2-07` je `READY` (neimplementováno)**; `R2-08` zůstává `NOT_READY` do dokončení R2-07. Další kanonický krok:
