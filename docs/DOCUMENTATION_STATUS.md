...
**`R2-01` až `R2-06` jsou implementovány; `R2-07` zůstává `NOT_READY` (čeká na kontrakt C15); `R2-08` zůstává `NOT_READY` (čeká na dokončení R2-07).**
...
**Přesný další kanonický krok:** `R2-01` až `R2-06` jsou implementovány. Další krok je příprava **posledního kontraktu C15 – Local-to-account migration** (`docs/12-data/r2-local-to-account-migration-contract.md` — anonymous→account attach, duplicitní ochrana, stabilita lokálních ID, seed vs user data), který odblokuje `R2-07 – Local-to-Account Data Migration`. Po R2-07 následuje `R2-08` (kritická E2E evidence + R2 Exit Review, vč. dlužné emulátorové runtime evidence). Implementace `R2-07` smí začít až po vzniku C15, Ready kontrole a samostatném pokynu.
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. **C1–C14 jsou kompletní**; z kontraktní mapy zbývá jen C15. **`R2-01` až `R2-06` jsou implementovány**; `R2-07` (čeká na C15) a `R2-08` zůstávají `NOT_READY`. Další kanonický krok:

```text
C15 – Local-to-account migration  (docs/12-data/r2-local-to-account-migration-contract.md)  [pro R2-07]
poté: implementace R2-07 – Local-to-Account Data Migration
poté: R2-08 – R2 Critical End-to-End Evidence and Exit Review
```

Implementace `R2-07` smí začít až po samostatném pokynu; před ní je nutné načíst aktuální GitHub, ověřit skutečnou strukturu repozitáře a provést Ready kontrolu podle `r2-vertical-slice-plan.md`, `definition-of-ready-and-done.md` a `coding-agent-guide.md`.
