...
**`R2-01`, `R2-02` a `R2-03` jsou implementovány; `R2-04` zůstává `NOT_READY` (čeká na kontrakty C8, C9 a rozšíření C6); `R2-05` až `R2-08` zůstávají `NOT_READY`.**
...
**Vytvořené R2 kontrakty:** C1 – Mobile schema migration (`docs/12-data/r2-mobile-schema-migration.md`, Data Architecture, `MSM-001..015`), C2 – Local ownership & outbox (`docs/12-data/r2-local-sync-metadata-contract.md`, Domain / sync-and-offline-model, `LSM-001..015`), C3 – Identity & session (`docs/07-backend/r2-identity-session-contract.md`, Domain / identity-and-profile-model + Backend, `ISC-001..015`), C4 – Authentication API (`docs/07-backend/r2-auth-api-contract.md`, Backend Architecture, `AAC-001..015`), C5 – Auth provider ADR (**`ADR-011`** v `docs/05-architecture/initial-architecture-decisions.md`, Architecture), C6 – Server data model (`docs/12-data/r2-server-data-model.md`, Data Architecture, `SDM-001..015`) C7 – Token/session storage (`docs/11-security/r2-token-session-storage-contract.md`, Security + Mobile, `TSS-001..015`; klasifikace session materiálu, secure storage boundary, restart/logout/revocation chování) a C14 – Audit-event contract (`docs/11-security/r2-audit-event-contract.md`, Domain / domain-events + Security, `AEC-001..015`; auth část Done, sync část forward-scoped před R2-05). Vše contract-only. `R2-01` (blocking C1, C2), `R2-02` (blocking C3, C4, C5, C6, auth část C14) i `R2-03` (blocking C7) jsou **implementovány** (viz výše). `R2-04` má Ready podmínku R2-03 Done + C8, C9 a rozšíření C6 o profil/device — kontrakty zatím neexistují → **`R2-04` je `NOT_READY`**. **Stále chybějící blokující kontrakty (pro pozdější R2 slices):** authorization/ownership (C8 → R2-04/05), device registration (C9 → R2-04), sync protocol (C10 → R2-05), idempotency (C11 → R2-05), conflict/rejection (C12 → R2-06), revocation (C13 → R2-06), sync část C14 (→ R2-05), local-to-account migration (C15 → R2-07). Tyto vzniknou nejpozději před slicem, který je používá.

**Přesný další kanonický krok:** `R2-01` až `R2-03` jsou implementovány. Další krok je příprava kontraktů **C8 – Authorization/ownership** (`docs/11-security/r2-authorization-ownership-contract.md`), **C9 – Device registration** (`docs/07-backend/r2-device-registration-contract.md`) a append-only rozšíření **C6** o profil/device, které společně odblokují `R2-04 – AthleteProfile and Device Registration`. Implementace `R2-04` smí začít až po vzniku kontraktů, Ready kontrole a samostatném pokynu.
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. C1–C7 a auth část C14 jsou vytvořené. **`R2-01` až `R2-03` jsou implementovány**; `R2-04` až `R2-08` zůstávají `NOT_READY` (R2-04 čeká na C8, C9 a rozšíření C6). Další kanonický krok:
...
+ append-only rozšíření C6 o profil/device
poté: implementace R2-04 – AthleteProfile and Device Registration
```

Implementace `R2-04` smí začít až po samostatném pokynu; před ní je nutné načíst aktuální GitHub, ověřit skutečnou strukturu repozitáře a provést Ready kontrolu podle `r2-vertical-slice-plan.md`, `definition-of-ready-and-done.md` a `coding-agent-guide.md`.
