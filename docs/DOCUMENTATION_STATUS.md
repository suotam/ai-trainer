...
**`R2-01` i `R2-02` jsou `READY` (neimplementováno); `R2-03` až `R2-08` zůstávají `NOT_READY`.** **Vytvořené R2 kontrakty:** C1 – Mobile schema migration (`docs/12-data/r2-mobile-schema-migration.md`, Data Architecture, `MSM-001..015`), C2 – Local ownership & outbox (`docs/12-data/r2-local-sync-metadata-contract.md`, Domain / sync-and-offline-model, `LSM-001..015`), C3 – Identity & session (`docs/07-backend/r2-identity-session-contract.md`, Domain / identity-and-profile-model + Backend, `ISC-001..015`), C4 – Authentication API (`docs/07-backend/r2-auth-api-contract.md`, Backend Architecture, `AAC-001..015`), C5 – Auth provider ADR (**`ADR-011`** v `docs/05-architecture/initial-architecture-decisions.md`, Architecture), C6 – Server data model (`docs/12-data/r2-server-data-model.md`, Data Architecture, `SDM-001..015`) a C14 – Audit-event contract (`docs/11-security/r2-audit-event-contract.md`, Domain / domain-events + Security, `AEC-001..015`; auth část Done, sync část forward-scoped před R2-05). Vše contract-only. `R2-01` má blocking kontrakty C1, C2 (oba hotové) → `READY`. `R2-02` má blocking kontrakty C3, C4, C5, C6 a auth část C14 (všechny hotové) → **`R2-02` je `READY`**. **Stále chybějící blokující kontrakty (pro pozdější R2 slices):** token/session storage (C7 → R2-03), authorization/ownership (C8 → R2-04/05), device registration (C9 → R2-04), sync protocol (C10 → R2-05), idempotency (C11 → R2-05), conflict/rejection (C12 → R2-06), revocation (C13 → R2-06), sync část C14 (→ R2-05), local-to-account migration (C15 → R2-07). Tyto vzniknou nejpozději před slicem, který je používá.

**Přesný další kanonický dokumentační krok:** C1–C6 a auth část C14 jsou vytvořené; **`R2-01` i `R2-02` jsou `READY` (neimplementováno)**. Další krok je buď **implementace R2-01/R2-02** (produkční kód — samostatné rozhodnutí), nebo příprava kontraktu **C7 – Token/session storage** (`docs/11-security/r2-token-session-storage-contract.md`) pro `R2-03`. Implementace R2 smí začít až po samostatném pokynu.
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. C1–C6 a auth část C14 jsou vytvořené. **`R2-01` i `R2-02` jsou `READY` (neimplementováno)**; `R2-03` až `R2-08` zůstávají `NOT_READY`. Další kanonický krok je buď implementace R2-01/R2-02 (produkční kód — samostatné rozhodnutí), nebo příprava kontraktu:
