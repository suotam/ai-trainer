...
**`R2-01` až `R2-05` jsou implementovány; `R2-06` zůstává `NOT_READY` (čeká na kontrakty C12 a C13); `R2-07` a `R2-08` zůstávají `NOT_READY`.**
...
**Přesný další kanonický krok:** `R2-01` až `R2-05` jsou implementovány. Další krok je příprava kontraktů **C12 – Conflict/rejection** (`docs/07-backend/r2-conflict-rejection-contract.md` nebo rozšíření C10) a **C13 – Revocation** (rozšíření C3/C7 nebo samostatný dokument), které odblokují `R2-06 – Conflict, Rejection and Session Revocation`. Implementace `R2-06` smí začít až po vzniku kontraktů, Ready kontrole a samostatném pokynu.
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. C1–C11 (vč. rozšíření C6 §8.1–§8.5) a kompletní C14 (auth i sync část) jsou vytvořené. **`R2-01` až `R2-05` jsou implementovány**; `R2-06` až `R2-08` zůstávají `NOT_READY` (R2-06 čeká na C12 a C13). Další kanonický krok:

```text
C12 – Conflict/rejection  (docs/07-backend/r2-conflict-rejection-contract.md)  [pro R2-06]
C13 – Revocation          (rozšíření C3/C7, nebo samostatný dokument)          [pro R2-06]
