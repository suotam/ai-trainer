...
**`R2-01` a `R2-02` jsou implementovány; `R2-03` zůstává `NOT_READY` (čeká na kontrakt C7); `R2-04` až `R2-08` zůstávají `NOT_READY`.**
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. C1–C6 a auth část C14 jsou vytvořené. **`R2-01` i `R2-02` jsou implementovány**; `R2-03` až `R2-08` zůstávají `NOT_READY` (R2-03 čeká na C7). Další kanonický krok je příprava kontraktu:

```text
C7 – Token/session storage  (docs/11-security/r2-token-session-storage-contract.md)  [odblokuje R2-03]
poté: implementace R2-03 – Mobile Auth and Secure Session Storage
```
