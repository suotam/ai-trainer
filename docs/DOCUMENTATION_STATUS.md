...
**`R2-01`, `R2-02` a `R2-03` jsou implementovány; kontrakty C8, C9 i rozšíření C6 existují → `R2-04` je `READY` (neimplementováno); `R2-05` až `R2-08` zůstávají `NOT_READY`.**
...
**Přesný další kanonický krok:** `R2-01` až `R2-03` jsou implementovány; C8, C9 i rozšíření C6 existují → **`R2-04` je `READY`**. Další krok je **implementace `R2-04 – AthleteProfile and Device Registration`** (backend Flyway V3 dle C6 §8.1–§8.3, profil/device endpointy s ownership enforcementem dle C8/C9, minimální mobilní UI), případně poté příprava kontraktů **C10 – Sync protocol** a **C11 – Idempotency** pro `R2-05`. Implementace `R2-04` smí začít až po Ready kontrole a samostatném pokynu.
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. C1–C9 (vč. rozšíření C6 §8.1–§8.3) a auth část C14 jsou vytvořené. **`R2-01` až `R2-03` jsou implementovány**; **`R2-04` je `READY` (neimplementováno)**; `R2-05` až `R2-08` zůstávají `NOT_READY`. Další kanonický krok:

```text
R2-04 – AthleteProfile and Device Registration  (implementace, dle C6 §8/C8/C9)
poté: C10 – Sync protocol, C11 – Idempotency, rozšíření C6 o synced entity, sync část C14  [pro R2-05]
```
