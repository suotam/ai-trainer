...
**`R2-01` až `R2-04` jsou implementovány; kontrakty C10, C11, rozšíření C6 §8.4–§8.5 i sync část C14 existují → `R2-05` je `READY` (neimplementováno); `R2-06` až `R2-08` zůstávají `NOT_READY`.**
...
Celé R1 (`R1-01` až `R1-08`) je implementované, R1 Exit Review je proveden (viz §3) a Release 1 je uzavřen. Existuje R2 vertical-slice plán. C1–C11 (vč. rozšíření C6 §8.1–§8.5) a kompletní C14 (auth i sync část) jsou vytvořené. **`R2-01` až `R2-04` jsou implementovány**; **`R2-05` je `READY` (neimplementováno)**; `R2-06` až `R2-08` zůstávají `NOT_READY`. Další kanonický krok:
