...
object RequestIdSupport {
    ...
    fun isValid(candidate: String?): Boolean = candidate != null && allowedPattern.matches(candidate)

    /** Vrátí validní příchozí hodnotu, jinak nový vygenerovaný identifikátor. */
    fun resolve(candidate: String?): String = if (isValid(candidate)) candidate!! else generate()
