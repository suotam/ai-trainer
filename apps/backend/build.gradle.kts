...
// Kotlin formátovací a style gate (R0-06, test-strategy §4). Verzované
// v Gradle, spustitelné lokálně i v CI stejným příkazem, bez globální
// instalace. CI kód neopravuje — ktlintFormat je pouze lokální pomůcka.
...
tasks.register<JavaExec>("ktlintCheck") {
    group = "verification"
    description = "Kontrola Kotlin formátování a stylu (ktlint)."
    classpath = ktlint
...
tasks.register<JavaExec>("ktlintFormat") {
    group = "formatting"
    description = "Lokální oprava Kotlin formátování (ktlint -F)."
    classpath = ktlint
    mainClass = "com.pinterest.ktlint.Main"
    jvmArgs("--add-opens=java.base/java.lang=ALL-UNNAMED")
    args("-F", "src/**/*.kt", "*.kts")
